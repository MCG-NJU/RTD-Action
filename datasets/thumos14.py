    ...
    def __init__(self, feature_folder, tem_folder, anno_file, split, args):
        ...
        for vid in video_list:
            if self.split in vid:
                ...
                annotations = [
                    item["segment_frame"] for item in self.anno_file[vid]["annotations"]
                ]
                labels = [
                    int(item["label"]) for item in self.anno_file[vid]["annotations"]
                ]
                ...
                if seq_len <= self.window_size:
                    ...
                    gt = [
                        (annotations[idx], labels[idx])
                        for idx in range(len(annotations))
                    ]
                    self.video_list.append(
                        VideoRecord(
                            vid, num_frames, locations, gt, s_e_scores, fps, args
                        )
                    )
                else:
                    if self.split == "test":
                    ...
                    ws_starts = [
                        i * stride
                        for i in range(
                            (seq_len // self.window_size - 1) * overlap_ratio + 1
                        )
                    ]
                    ...
                    for ws in ws_starts:
                        ...
                        s_e_scores = np.concatenate((s_scores, e_scores), axis=1)
...
def build(split, args):
    ...
    dataset = ThumosDetection(feature_folder, tem_folder, anno_file, split, args)
    return dataset
