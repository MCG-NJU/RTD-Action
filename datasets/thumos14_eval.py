...
def average_recall_vs_nr_proposals(
    proposals, ground_truth, tiou_thresholds=np.linspace(0.5, 1.0, 11)
):
    ...
    for videoid in video_lst:
        ...
        this_video_proposals = proposals[prop_idx][["f-init", "f-end"]].values.astype(
            np.float
        )
    ...
    for ridx, tiou in enumerate(tiou_thresholds):
        ...
        for i, score in enumerate(score_lst):
            ...
            for j, pcn in enumerate(pcn_lst):
                ...
                matches[i, j] = (
                    (score[:, :nr_proposals] >= tiou).sum(axis=1) > 0
                ).sum()
...
def formatting(results):
    ...
    results_pd = pd.DataFrame(
        results_list, columns=["f-init", "f-end", "score", "video-frames", "video-name"]
    )
    return results_pd
