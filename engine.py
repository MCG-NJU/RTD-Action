...
from datasets.anet_eval import ANetEvaluator
...
def evaluate(model, criterion, postprocessors, data_loader, device, args):
    ...
    for (
        ...
        num_frames,
        s_e_scores,
    ) in metric_logger.log_every(data_loader, 10, header):
        ...
        results = postprocessors["bbox"](outputs, num_frames)

        for target, output in zip(targets, results):
            vid = video_dict[target["video_id"].item()]
            anet_evaluator.update(vid, output)
        # break

    # gather the stats from all processes
    metric_logger.synchronize_between_processes()
    anet_evaluator.synchronize_between_processes()
    print("Averaged stats:", metric_logger)

    return anet_evaluator, loss_dict
