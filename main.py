...
import datasets.anet as anet
import util.misc as utils
from datasets import build_dataset
from datasets.anet_eval import eval_props
...
def get_args_parser():
    ...
    parser.add_argument("--relax_rule", default="thresh", type=str)
    parser.add_argument("--relax_thresh", default=0.7, type=float)

    # dataset parameters
    parser.add_argument("--dataset_file", default="anet")
    ...
    parser.add_argument(
        "--tem_path",
        default="data/TEM_scores",
        type=str,
    )
    parser.add_argument(
        "--annotation_path", default="datasets/anet_anno_action.json", type=str
    )
    ...
    parser.add_argument("--stage", default=1, type=int, help="stage-id of RTD-Net")

    parser.add_argument("--csv_path", default="datasets/video_info_new.csv", type=str)
    parser.add_argument("--temporal_scale", default=100, type=int)
...
def main(args):
    ...
    if args.stage == 1:
        ...
        learned_params = filter(
            lambda p: p.requires_grad, model_without_ddp.parameters()
        )
    elif args.stage == 2:
        for name, value in model_without_ddp.named_parameters():
            if "class_embed" not in name:
                value.requires_grad = False
        head_params = filter(lambda p: p.requires_grad, model_without_ddp.parameters())
    ...
    optimizer = torch.optim.AdamW(
        learned_params, lr=args.lr, weight_decay=args.weight_decay
    )
    ...
    if args.eval:
        evaluator, eval_loss_dict = evaluate(
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if epoch % 50 == 0 and args.output_dir:
            checkpoint_path = output_dir / "checkpoint_epoch{}.pth".format(epoch)
        ...
        if float(test_stats["1"]) > best_ar1:
            best_ar1 = float(test_stats["1"])
            with (output_dir / "log_best_ar1.txt").open("w") as f:
                f.write(json.dumps(log_stats) + "\n")
            checkpoint_path = output_dir / "checkpoint_best_ar1.pth"
            utils.save_on_master(
                {
                    "model": model_without_ddp.state_dict(),
                    "optimizer": optimizer.state_dict(),
        ...
        if float(test_stats["100"]) > best_ar100:
            best_ar100 = float(test_stats["100"])
            with (output_dir / "log_best_ar100.txt").open("w") as f:
                f.write(json.dumps(log_stats) + "\n")
            checkpoint_path = output_dir / "checkpoint_best_ar100.pth"
            utils.save_on_master(
                {
                    "model": model_without_ddp.state_dict(),
                    "optimizer": optimizer.state_dict(),
        ...
        current_sum_ar = (
            float(test_stats["1"]) + float(test_stats["10"]) + float(test_stats["100"])
        )
        ...
        current_auc = float(test_stats["auc"])
        if current_auc > best_auc:
            best_auc = current_auc
            with (output_dir / "log_best_auc.txt").open("a+") as f:
                f.write(json.dumps(log_stats) + "\n")
            checkpoint_path = output_dir / "checkpoint_best_auc.pth"
            results_pd.to_csv(args.output_dir + f"results_best_auc.csv")
            utils.save_on_master(
                {
                    "model": model_without_ddp.state_dict(),
                    "optimizer": optimizer.state_dict(),
